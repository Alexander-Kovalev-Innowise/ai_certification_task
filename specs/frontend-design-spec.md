...
`CreateTrainerModal` posts `CreateTrainerDto { businessName, trainerName, email, phone }` — see §11 open item on the `trainerName` single-field-vs-split question.
...
| `resetPasswordSchema` | `ResetPasswordDto` | `newPassword`: min 12, matches the same `PASSWORD_POLICY` pattern class documented server-side (mirrored constant, not re-derived — see §11 open item if the server's regex isn't exported anywhere the client can import) |
...
| `createTrainerSchema` | `CreateTrainerDto` | `businessName` max 200, `trainerName` max 100, `email` valid, `phone`: E.164-ish pattern matching `@IsPhoneNumber()` |
...
|---|---|---|---|
| 11.1 | `CreateTrainerDto.trainerName` is a single server-side field, but `UserSummaryDto`/`MeResponseDto` elsewhere expose `firstName`/`lastName` separately (api-spec §4.1 itself flags this: "split server-side, or kept as one field — confirm with frontend-design"). | `CreateTrainerModal` designed with a **single** "Trainer Name" input matching the DTO literally, to avoid inventing a client-side split/join convention the server doesn't document (would the server split on the first space? Reject multi-word first names?). | Confirm single-field is acceptable for the create-trainer UI, or specify the exact split rule so the modal can offer two inputs instead. |
...
| 11.5 | Password policy regex (`PASSWORD_POLICY`, referenced but not spelled out in `api-designer-spec.md`'s `@Matches(PASSWORD_POLICY)`) has no concrete pattern documented anywhere upstream. | Client schemas (§7.1) assume "min 12 chars" only, with the exact character-class rule left as a placeholder import pending the server exposing (or this spec being told) the literal regex — mirroring an undefined pattern risks the two silently diverging. | Needs the literal `PASSWORD_POLICY` regex from `api-designer`/`architect` (or confirmation that "min 12, no further constraint" is in fact the whole rule) before `coder-frontend` implements `loginSchema`'s sibling password schemas. |
